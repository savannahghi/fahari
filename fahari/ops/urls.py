from django.urls import path

from .views import (
    ActivityLogCreateView,
    ActivityLogDeleteView,
    ActivityLogUpdateView,
    ActivityLogView,
    CommoditiesListView,
    CommodityCreateView,
    CommodityDeleteView,
    CommodityUpdateView,
    DailySiteUpdatesView,
    DailyUpdateCreateView,
    DailyUpdateDeleteView,
    DailyUpdateUpdateView,
    FacilityDeviceCreateView,
    FacilityDeviceDeleteView,
    FacilityDeviceRequestCreateView,
    FacilityDeviceRequestDeleteView,
    FacilityDeviceRequestsListView,
    FacilityDeviceRequestUpdateView,
    FacilityDevicesListView,
    FacilityDeviceUpdateView,
    FacilityNetworkStatusCreateView,
    FacilityNetworkStatusDeleteView,
    FacilityNetworkStatusListView,
    FacilityNetworkStatusUpdateView,
    FacilityQuestionnaireCreateView,
    FacilityQuestionnaireDeleteView,
    FacilityQuestionnaireUpdateView,
    FacilitySystemCreateView,
    FacilitySystemDeleteView,
    FacilitySystemsView,
    FacilitySystemTicketCreateView,
    FacilitySystemTicketDeleteView,
    FacilitySystemTicketResolveView,
    FacilitySystemTicketsView,
    FacilitySystemTicketUpdateView,
    FacilitySystemUpdateView,
    MentorshipQuestionnaireListView,
    MentorshipTeamCreateView,
    MentorshipTeamDeleteView,
    MentorshipTeamUpdateView,
    QuestionCreateView,
    QuestionDeleteView,
    QuestionGroupCreateView,
    QuestionGroupDeleteView,
    QuestionGroupListView,
    QuestionGroupUpdateView,
    QuestionListView,
    QuestionnaireCreateView,
    QuestionnaireDeleteView,
    QuestionnaireListView,
    QuestionnaireSectionView,
    QuestionnaireUpdateView,
    QuestionUpdateView,
    SecurityIncidenceCreateView,
    SecurityIncidenceDeleteView,
    SecurityIncidenceUpdateView,
    SecurityIncidentsListView,
    SiteMentorshipCreateView,
    SiteMentorshipDeleteView,
    SiteMentorshipUpdateView,
    SiteMentorshipView,
    StockReceiptVerificationCreateView,
    StockReceiptVerificationDeleteView,
    StockReceiptVerificationUpdateView,
    StockReceiptVerificationView,
    TimeSheetApproveView,
    TimeSheetCreateView,
    TimeSheetDeleteView,
    TimeSheetsView,
    TimeSheetUpdateView,
    UoMCategoryCreateView,
    UoMCategoryDeleteView,
    UoMCategoryListView,
    UoMCategoryUpdateView,
    UoMCreateView,
    UoMDeleteView,
    UoMListView,
    UoMUpdateView,
    WeeklyProgramUpdateCommentsView,
    WeeklyProgramUpdatesCommentCreateView,
    WeeklyProgramUpdatesCommentDeleteView,
    WeeklyProgramUpdatesCommentUpdateView,
    WeeklyProgramUpdatesCreateView,
    WeeklyProgramUpdatesDeleteView,
    WeeklyProgramUpdatesUpdateView,
    WeeklyProgramUpdatesView,
)

app_name = "ops"
urlpatterns = [
    path("versions", view=FacilitySystemsView.as_view(), name="versions"),
    path(
        "version_create",
        view=FacilitySystemCreateView.as_view(),
        name="version_create",
    ),
    path(
        "version_update/<pk>",
        view=FacilitySystemUpdateView.as_view(),
        name="version_update",
    ),
    path(
        "version_delete/<pk>",
        view=FacilitySystemDeleteView.as_view(),
        name="version_delete",
    ),
    path("tickets", view=FacilitySystemTicketsView.as_view(), name="tickets"),
    path(
        "ticket_create",
        view=FacilitySystemTicketCreateView.as_view(),
        name="ticket_create",
    ),
    path(
        "ticket_update/<pk>",
        view=FacilitySystemTicketUpdateView.as_view(),
        name="ticket_update",
    ),
    path(
        "ticket_delete/<pk>",
        view=FacilitySystemTicketDeleteView.as_view(),
        name="ticket_delete",
    ),
    path(
        "ticket_resolve/<pk>",
        view=FacilitySystemTicketResolveView.as_view(),
        name="ticket_resolve",
    ),
    path("activity_logs", view=ActivityLogView.as_view(), name="activity_logs"),
    path(
        "activity_log_create",
        view=ActivityLogCreateView.as_view(),
        name="activity_log_create",
    ),
    path(
        "activity_log_update/<pk>",
        view=ActivityLogUpdateView.as_view(),
        name="activity_log_update",
    ),
    path(
        "activity_log_delete/<pk>",
        view=ActivityLogDeleteView.as_view(),
        name="activity_log_delete",
    ),
    path("site_mentorships", view=SiteMentorshipView.as_view(), name="site_mentorships"),
    path(
        "site_mentorship_create",
        view=SiteMentorshipCreateView.as_view(),
        name="site_mentorship_create",
    ),
    path(
        "site_mentorship_update/<pk>",
        view=SiteMentorshipUpdateView.as_view(),
        name="site_mentorship_update",
    ),
    path(
        "site_mentorship_delete/<pk>",
        view=SiteMentorshipDeleteView.as_view(),
        name="site_mentorship_delete",
    ),
    path(
        "daily_site_updates",
        view=DailySiteUpdatesView.as_view(),
        name="daily_site_updates",
    ),
    path(
        "daily_update_create",
        view=DailyUpdateCreateView.as_view(),
        name="daily_update_create",
    ),
    path(
        "daily_update_update/<pk>",
        view=DailyUpdateUpdateView.as_view(),
        name="daily_update_update",
    ),
    path(
        "daily_update_delete/<pk>",
        view=DailyUpdateDeleteView.as_view(),
        name="daily_update_delete",
    ),
    path(
        "weekly_program_updates",
        view=WeeklyProgramUpdatesView.as_view(),
        name="weekly_program_updates",
    ),
    path(
        "weekly_program_updates_create",
        view=WeeklyProgramUpdatesCreateView.as_view(),
        name="weekly_program_updates_create",
    ),
    path(
        "weekly_program_updates_update/<pk>",
        view=WeeklyProgramUpdatesUpdateView.as_view(),
        name="weekly_program_updates_update",
    ),
    path(
        "weekly_program_updates_delete/<pk>",
        view=WeeklyProgramUpdatesDeleteView.as_view(),
        name="weekly_program_updates_delete",
    ),
    path(
        "weekly_program_update_comments",
        view=WeeklyProgramUpdateCommentsView.as_view(),
        name="weekly_program_update_comments",
    ),
    path(
        "weekly_program_update_comments_create",
        view=WeeklyProgramUpdatesCommentCreateView.as_view(),
        name="weekly_program_update_comments_create",
    ),
    path(
        "weekly_program_update_comments_update/<pk>",
        view=WeeklyProgramUpdatesCommentUpdateView.as_view(),
        name="weekly_program_update_comments_update",
    ),
    path(
        "weekly_program_update_comments_delete/<pk>",
        view=WeeklyProgramUpdatesCommentDeleteView.as_view(),
        name="weekly_program_update_comments_delete",
    ),
    path("timesheets", view=TimeSheetsView.as_view(), name="timesheets"),
    path(
        "timesheet_create",
        view=TimeSheetCreateView.as_view(),
        name="timesheet_create",
    ),
    path(
        "timesheet_update/<pk>",
        view=TimeSheetUpdateView.as_view(),
        name="timesheet_update",
    ),
    path(
        "timesheet_delete/<pk>",
        view=TimeSheetDeleteView.as_view(),
        name="timesheet_delete",
    ),
    path(
        "timesheet_approve/<pk>",
        view=TimeSheetApproveView.as_view(),
        name="timesheet_approve",
    ),
    path(
        "stock_receipt_verifications",
        view=StockReceiptVerificationView.as_view(),
        name="stock_receipt_verifications",
    ),
    path(
        "stock_receipt_verification_create",
        view=StockReceiptVerificationCreateView.as_view(),
        name="stock_receipt_verification_create",
    ),
    path(
        "stock_receipt_verification_update/<pk>",
        view=StockReceiptVerificationUpdateView.as_view(),
        name="stock_receipt_verification_update",
    ),
    path(
        "stock_receipt_verification_delete/<pk>",
        view=StockReceiptVerificationDeleteView.as_view(),
        name="stock_receipt_verification_delete",
    ),
    path("commodities", view=CommoditiesListView.as_view(), name="commodities"),
    path(
        "commodity_create",
        view=CommodityCreateView.as_view(),
        name="commodity_create",
    ),
    path(
        "commodity_update/<pk>",
        view=CommodityUpdateView.as_view(),
        name="commodity_update",
    ),
    path(
        "commodity_delete/<pk>",
        view=CommodityDeleteView.as_view(),
        name="commodity_delete",
    ),
    path("uoms", view=UoMListView.as_view(), name="uoms"),
    path("uom_create", view=UoMCreateView.as_view(), name="uom_create"),
    path("uom_update/<pk>", view=UoMUpdateView.as_view(), name="uom_update"),
    path("uom_delete/<pk>", view=UoMDeleteView.as_view(), name="uom_delete"),
    path("uom_categories", view=UoMCategoryListView.as_view(), name="uom_categories"),
    path("uom_category_create", view=UoMCategoryCreateView.as_view(), name="uom_category_create"),
    path(
        "uom_category_update/<pk>",
        view=UoMCategoryUpdateView.as_view(),
        name="uom_category_update",
    ),
    path(
        "uom_category_delete/<pk>",
        view=UoMCategoryDeleteView.as_view(),
        name="uom_category_delete",
    ),
    path(
        "facility_network_status",
        view=FacilityNetworkStatusListView.as_view(),
        name="facility_network_status",
    ),
    path(
        "network_status_create",
        view=FacilityNetworkStatusCreateView.as_view(),
        name="network_status_create",
    ),
    path(
        "network_status_update/<pk>",
        view=FacilityNetworkStatusUpdateView.as_view(),
        name="network_status_update",
    ),
    path(
        "network_status_delete/<pk>",
        view=FacilityNetworkStatusDeleteView.as_view(),
        name="network_status_delete",
    ),
    path(
        "facility_devices",
        view=FacilityDevicesListView.as_view(),
        name="facility_devices",
    ),
    path(
        "facility_device_create",
        view=FacilityDeviceCreateView.as_view(),
        name="facility_device_create",
    ),
    path(
        "facility_device_update/<pk>",
        view=FacilityDeviceUpdateView.as_view(),
        name="facility_device_update",
    ),
    path(
        "facility_device_delete/<pk>",
        view=FacilityDeviceDeleteView.as_view(),
        name="facility_device_delete",
    ),
    path(
        "facility_device_requests",
        view=FacilityDeviceRequestsListView.as_view(),
        name="facility_device_requests",
    ),
    path(
        "facility_device_request_create",
        view=FacilityDeviceRequestCreateView.as_view(),
        name="facility_device_request_create",
    ),
    path(
        "facility_device_request_update/<pk>",
        view=FacilityDeviceRequestUpdateView.as_view(),
        name="facility_device_request_update",
    ),
    path(
        "facility_device_request_delete/<pk>",
        view=FacilityDeviceRequestDeleteView.as_view(),
        name="facility_device_request_delete",
    ),
    path(
        "security_incidents",
        view=SecurityIncidentsListView.as_view(),
        name="security_incidents",
    ),
    path(
        "security_incidence_create",
        view=SecurityIncidenceCreateView.as_view(),
        name="security_incidence_create",
    ),
    path(
        "security_incidence_update/<pk>",
        view=SecurityIncidenceUpdateView.as_view(),
        name="security_incidence_update",
    ),
    path(
        "security_incidence_delete/<pk>",
        view=SecurityIncidenceDeleteView.as_view(),
        name="security_incidence_delete",
    ),
    path(
        "questions",
        view=QuestionListView.as_view(),
        name="questions",
    ),
    path(
        "question_create",
        view=QuestionCreateView.as_view(),
        name="question_create",
    ),
    path(
        "question_update/<pk>",
        view=QuestionUpdateView.as_view(),
        name="question_update",
    ),
    path(
        "question_delete/<pk>",
        view=QuestionDeleteView.as_view(),
        name="question_delete",
    ),
    path(
        "question_groups",
        view=QuestionGroupListView.as_view(),
        name="question_groups",
    ),
    path(
        "question_group_create",
        view=QuestionGroupCreateView.as_view(),
        name="question_group_create",
    ),
    path(
        "question_group_update/<pk>",
        view=QuestionGroupUpdateView.as_view(),
        name="question_group_update",
    ),
    path(
        "question_group_delete/<pk>",
        view=QuestionGroupDeleteView.as_view(),
        name="question_group_delete",
    ),
    path(
        "add_questionnaire",
        view=QuestionnaireSectionView.as_view(),
        name="add_questionnaire",
    ),
    path(
        "questionnaires",
        view=QuestionnaireListView.as_view(),
        name="questionnaires",
    ),
    path(
        "questionnaire_create",
        view=QuestionnaireCreateView.as_view(),
        name="questionnaire_create",
    ),
    path(
        "questionnaire_update/<pk>",
        view=QuestionnaireUpdateView.as_view(),
        name="questionnaire_update",
    ),
    path(
        "questionnaire_delete/<pk>",
        view=QuestionnaireDeleteView.as_view(),
        name="questionnaire_delete",
    ),
    path(
        "mentorship_questionnaires",
        view=MentorshipQuestionnaireListView.as_view(),
        name="mentorship_questionnaires",
    ),
    path(
        "facility_questionnaire_create",
        view=FacilityQuestionnaireCreateView.as_view(),
        name="facility_questionnaire_create",
    ),
    path(
        "facility_questionnaire_update/<pk>",
        view=FacilityQuestionnaireUpdateView.as_view(),
        name="facility_questionnaire_update",
    ),
    path(
        "facility_questionnaire_delete/<pk>",
        view=FacilityQuestionnaireDeleteView.as_view(),
        name="facility_questionnaire_delete",
    ),
    path(
        "mentorship_team_create",
        view=MentorshipTeamCreateView.as_view(),
        name="mentorship_team_create",
    ),
    path(
        "mentorship_team_update/<pk>",
        view=MentorshipTeamUpdateView.as_view(),
        name="mentorship_team_update",
    ),
    path(
        "mentorship_team_delete/<pk>",
        view=MentorshipTeamDeleteView.as_view(),
        name="mentorship_team_delete",
    ),
]
